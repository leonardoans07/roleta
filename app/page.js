...
export default function Home() {
  ...
  function analyzePattern() {
    ...
    if (strongest[1] >= 5) {
      ...
      // Dynamic: based on actual frequency in last 12
      confidence = Math.min(
        92,
        Math.round((strongest[1] / 12) * 100 + 30)
      );
    ...
    if (hotFinal[1] >= 3) {
      ...
      alert = `FINAL ${hotFinal[0]} HOT`;
    }

    // ===== SETOR DA RODA =====
    // Shows which physical sector of the wheel is statistically active.
    // Expected hits = sector_size / 37 * 12. Hot = 1.5x expected.

    const voisinsHits =
      recent.filter(n => VOISINS.includes(n)).length;

    const tiersHits =
      recent.filter(n => TIERS.includes(n)).length;

    const orphelinsHits =
      recent.filter(n => ORPHELINS.includes(n)).length;
    ...
    if (nonZero.length >= 8) {
      ...
      const redCount =
        nonZero.filter(n => RED.includes(n)).length;
    ...
    if (nonZero.length >= 8) {
      ...
      if (oddRatio >= 0.65)
        parityPrediction = {
          bet: "ODD",
          pct: Math.round(oddRatio * 100)
        };
      else if (oddRatio <= 0.35)
        parityPrediction = {
          bet: "EVEN",
          pct: Math.round((1 - oddRatio) * 100)
        };
    }
  ...
  return (
    ...
    >
      ...
      <div style={{ marginTop: 20 }}>
        ...
        {error && (
          <p style={{ color: "#ff6666", marginTop: 8 }}>
            ⚠ {error}
          </p>
        )}
          ...
          >
            ...
            {prediction.colorPrediction && (
              <div
                style={{
                  background:
                    prediction.colorPrediction.bet === "RED"
                      ? "#cc2233"
                      : "#333",
                  padding: "14px 24px",
                  borderRadius: 10,
              ...
              >
                {prediction.colorPrediction.bet}
                {" — "}
                {prediction.colorPrediction.pct}%
              </div>
            )}
            ...
            {prediction.parityPrediction && (
              ...
              >
                {prediction.parityPrediction.bet}
                {" — "}
                {prediction.parityPrediction.pct}%
              </div>
            )}
